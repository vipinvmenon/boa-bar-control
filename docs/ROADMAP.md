...
| BAR-185 | Above 520px, an application rather than a picture of a phone | claude | The desktop path rendered a 390×844 bezelled mock on a gradient field, captioned with its own pixel dimensions, and `.bars-body .bar-row-card` grew each card to fill the frame — 120px of card around 74px of text, which reads as data missing rather than a list that has ended. **Acceptance:** one column at a comfortable measure filling the height, no bezel, gradient or caption; list cards sized to their content; nothing below 520px changes; ADR-018 supersedes the desktop half of BAR-039 |
| BAR-181 | One type scale, and text that passes contrast | claude | 26 distinct font sizes across 240 declarations, 129 of them at 9–11px, most uppercase Oswald tracked out — so nothing led and hierarchy came from container borders rather than type. Inactive nav labels measured 3.89:1. **Acceptance:** one token scale, no raw pixel font size in the stylesheet, 13px floor for anything read as a sentence, every text colour ≥4.5:1 against the surface it actually sits on, and ADR-017 recording the deviation from the design's 9–10px labels |
| BAR-182 | Tone the confirm dialogs by what they actually destroy | claude | `tone="danger"` had never been passed by any caller, so BAR-178's danger treatment never rendered; and the receipt screen unmounted its own trigger while the dialog was open, so focus returned to `<body>`. **Acceptance:** each call site tones by consequence with the reason stated; all three dialogs return focus to their opener |
| BAR-183 | Stop navigating to a screen that only apologises | claude | `/reports` rendered three paragraphs explaining that no report exists. **Acceptance:** one sentence and a link to the screen that does answer the question; the route still resolves so bookmarks land |
| BAR-184 | Label the invite form, name the alert cards, warn about demo once | claude | Four placeholder-only controls; alert cards whose accessible name was the whole card concatenated; and the demo warning rendered twice on home. **Acceptance:** visible labels on every field, one-sentence `aria-label` per alert composed from its own fields, decorative pills `aria-hidden`, and exactly one demo warning on screen |
| BAR-180 | The issue screen must not propose emptying a SKU | claude | `initialContainers` returned `min(2 cases, full-case maximum)`, so for any SKU holding two cases or fewer the screen opened proposing ALL of it — Corona Extra holds exactly 48 in the fixture and read `48 → 0 bottles`, with 4 CS and 6 CS greyed out so the figure looked like a considered maximum. Two taps from issued. The 4 September audit recorded this as "quantity survives a product change", which was the wrong mechanism: the reset does happen, the default it resets TO was the defect. **Acceptance:** the default steps down a case rather than taking the whole position, and can still be raised to it; issuing the entire position says so on screen; a SKU with plenty in stock is unaffected |
