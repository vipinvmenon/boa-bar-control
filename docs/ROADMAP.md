...
| BAR-152 | **Delete every fabricated operational number** | codex | The invented `−2.1%`, `₹18.4K`, `94% Mapped POS`, the static run-out projection and the static docket-SLA figure are removed. A number nobody computed must not be on screen — an empty state is honest, a fabricated figure is not |
...
| BAR-008 | **Two-fixture-state** visual harness | codex | `pnpm test:visual` renders each screen from the fixture repository against **two distinct fixture states** and diffs both. This is the anti-hardcoding gate: a screen with literals produces identical output for both states and therefore fails. Nine measured assertions replace "visibly matches" |
